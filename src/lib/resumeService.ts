...
export async function getDefaultResume() {
  const response = await fetch(`${baseUrl}/api/public/resume`);
...
export async function getAllResumes(): Promise<Resume[]> {
  const response = await fetch(`${baseUrl}/api/public/resumes`);
  if (!response.ok) throw new Error(await response.text());
  return response.json();
}
...
export async function getResumeByID(id: string) {
  const response = await fetch(`${baseUrl}/api/public/resume/${id}`);
  if (!response.ok) throw new Error(await response.text());
  return response.json();
}
...
export async function makeNewResume(resume: CreateResume): Promise<Resume> {
  const response = await fetch(`${baseUrl}/api/admin/resume`, {
    method: "POST",
...
export async function editResumeByID(id: string, resume: Resume) {
  const response = await fetch(`${baseUrl}/api/admin/resume/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(resume),
  });
...
export async function deleteResumeByID(id: string): Promise<{ ok: true }> {
  const res = await fetch(`${baseUrl}/api/admin/resume/${id}`, {
    method: "DELETE",
  });
